...
export function createArticleCardModel(input: ArticleCardInput): ArticleCardModel {
	...
	const bylineParts = [
		input.author ? (input.locale === "en" ? `By ${input.author}` : input.author) : null,
...
export function articleCardMarkup(
	...
	>
		<div tw="flex items-center justify-between w-full pb-6 border-b" style="border-color: #3a332d;">
			<div tw="flex items-center">
				...
				<span
					...
					style="color: #d6a06c; font-family: ${fonts.meta};"
				>
					Astro Kami / ${model.sectionLabel}
				</span>
			</div>
		...
		<div tw="flex flex-col flex-1 pt-10">
			<h1
				tw="m-0"
				style="color: #fbf6ec; font-family: ${fonts.display}; font-size: ${titleType.fontSize}px; font-weight: 600; line-height: ${titleType.lineHeight};"
			>
				${model.title}
			</h1>
		...
		<div tw="flex items-end justify-between w-full pt-7 border-t" style="border-color: #3a332d;">
			...
			<span tw="text-lg tracking-wide" style="color: #6f655c; font-family: ${fonts.meta};">
				${model.host}
			</span>
		</div>
	</div>`;
